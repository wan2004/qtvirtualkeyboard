...
public:
    ...
    {
        // qDebug() << "### candidateAt index " << index;
    ...
    QSet<qint32> activeHardwareKeys;
};
...
{
    setObjectName(tr("zh_CN:PinyinInputMethod"));
...
bool PinyinInputMethod::keyEvent(Qt::Key key, const QString &text, Qt::KeyboardModifiers modifiers)
{
    ...
    // qDebug() << "### PinyinInputMethod::keyEvent = " << text << " key " << key
    //         << " d->state " << d->state;
    ...
    if (d->inputMode == QVirtualKeyboardInputEngine::InputMode::Pinyin) {
        ...
        } else if (d->state != PinyinInputMethodPrivate::Idle && key >= Qt::Key_1 && key <= Qt::Key_9 ){
            d->chooseAndUpdate(key - Qt::Key_1);
            return true;
...
void PinyinInputMethod::reset()
{
    ...
    if(inputContext())
    {
        connect(inputContext(),&QVirtualKeyboardInputContext::hardKeyBoardChanged,this, [&](qint32 keycode , qint32 keystate)
        {
            ...
            if(d->inputMode != QVirtualKeyboardInputEngine::InputMode::Pinyin 
                || !inputContext() 
                || (d->state != PinyinInputMethodPrivate::Input && (keycode == Qt::Key_Backspace || keycode == Qt::Key_Delete) )
                 )
                return false;
            ...
            else if( keystate == HKeyPress )
            {
                if(!d->activeHardwareKeys.contains(keycode))
                {
                    ...
                    // qDebug() << "### d->activeHardwareKeys  " << d->activeHardwareKeys
                    //          << " reval " << reval << " state = " << d->state;
